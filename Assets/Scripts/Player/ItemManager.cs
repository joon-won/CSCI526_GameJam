...
//        #region Fields
//        [ComputedFields]
//        [ShowInInspector] private Dictionary<Card, Item> configToItem = new();
//        [ShowInInspector] private Dictionary<ItemRank, List<Item>> rankToItem = new();

//        [SerializeField] private List<Card> itemConfigs;
//        #endregion
...
//        public Card GetRandom(ItemRank rank) {
...
//        public void Buy(Card config) {
//            var item = configToItem[config];
//            if (item.NumAvailable == 0) {
//                Debug.LogWarning($"Trying to buy an item {config.CardName} with 0 numAvailable. ");
//                return;
//            }
//            if (!Player.Instance.TryPay(config.Cost)) return;
...
//        /// </summary>
//        /// <param name="config">Config of the item. </param>
//        public void Sell(Card config) {
//            var item = configToItem[config];
//            if (item.NumStacked == 0) {
//                Debug.LogWarning($"Trying to sell an item {config.CardName} with 0 numStacked. ");
//                return;
//            }

//            item.Remove();
//            Player.Instance.AddGold(config.Cost);
...
//            itemConfigs = Utility.FindRefsInFolder<Card>(itemConfigsPath);
